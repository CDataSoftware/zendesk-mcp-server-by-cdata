...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Zendesk (beta)](https://www.cdata.com/download/download.aspx?sku=FZZK-V&type=beta). 
## Purpose
...
This server wraps that driver and makes Zendesk data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.

## Setup Guide
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/zendesk-mcp-server-by-cdata.git
      cd zendesk-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        ...
        * (Mac/Linux) `/Applications/CData JDBC Driver for Zendesk/`
    * Run the command `java -jar cdata.jdbc.zendesk.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.zendesk.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `zendesk.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.zendesk.ZendeskDriver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=zendesk
      ServerName=CDataZendesk
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.zendesk.jar
      DriverClass=cdata.jdbc.zendesk.ZendeskDriver
      JdbcUrl=jdbc:zendesk:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
